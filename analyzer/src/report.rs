...
impl Report {
    ...
    fn to_human_readable(&self) -> Result<String> {
        ...
        output.push_str(&format!("Imported Functions:     {}\n", self.metrics.imported_functions));
        output.push_str(&format!("Exported Functions:     {}\n", self.metrics.exported_functions));
        output.push_str(&format!("Code Section Size:      {} bytes\n", self.metrics.code_section_size));
        output.push_str(&format!("Total Module Size:      {} bytes\n", self.metrics.total_module_size));
        output.push_str(&format!("Total Imports:          {}\n", self.metrics.imports_count));
        output.push_str(&format!("Total Exports:          {}\n", self.metrics.exports_count));
        ...
        output.push_str(&format!("Gas Risk Level:         {}\n", self.performance.gas_risk_level));
